...
int main()
{
	const int nqubits = 8;
	const int nlayers = 4;
	const int ulayers = 60;

	// read initial data from disk
	char filename[1024];
	sprintf(filename, "./input_data/hessian_bench_inti_n%i_q%i.hdf5", nlayers, nqubits);
    ...
    ret = ufunc_matfree(&psi, udata, &Upsi);
    if (ret < 0) {
    ...
    if (matchgate_brickwall_unitary_forward(vlist, nlayers, pperms, &psi, &cache, &Wpsi) < 0) {
    ...
    int ret = matchgate_brickwall_unitary_backward_hessian(vlist, nlayers, pperms, cache,
