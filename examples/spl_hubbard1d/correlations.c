...
{   
    ...
    char filename[1024];
	sprintf(filename, "../examples/spl_hubbard1d/input/spl_hubbard1d_suzuki%i_n%i_q%i_u%i_t%.2fs_g%.1f_init.hdf5", order, nlayers, nqubits, ulayers, delta, g);
    ...
    for (int t = 0; t < num_steps; t++) {
        ...
        for (int j = 0; j < nqubits; j++) {
            apply_oneqgate(&number_operator, j, &Npsi, &Jpsi);
    ...
    sprintf(filename, "../examples/spl_hubbard1d/spl_hubbard1d_g%.1f_correlations.hdf5", g);
