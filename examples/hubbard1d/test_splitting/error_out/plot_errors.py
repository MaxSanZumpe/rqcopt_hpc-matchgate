...
file_path = os.path.join(script_dir, f"hubbard1d_suzuki2_q{q}_us{63}_u{253}_t{t:.2f}s_g{g:.2f}_errors.hdf5")
...
file_path = os.path.join(script_dir, f"hubbard1d_suzuki4_q{q}_us{13}_u{261}_t{t:.2f}s_g{g:.2f}_errors.hdf5")
with h5py.File(file_path, "r") as f:
    errors = np.array(f["errors"])
    layers = np.array(f["layers"])
...
ax.set_title(f"Hubbard (1D):  J = {1}, U = {g}, t = {t}s")
ax.legend()
