...
int main()
{
	...
	optimize_matchgate_brickwall_circuit_hmat(func, udata, vlist_start, nlayers, nqubits, pperms, &params, niter, f_iter, f_citer, vlist_opt);
	...
	hsize_t fcdims[2] = { niter + 1, 2 };
	if (write_hdf5_dataset(file, "f_citer", 2, fcdims, H5T_IEEE_F64LE, H5T_NATIVE_DOUBLE, f_citer) < 0) {
		fprintf(stderr, "writing 'f_citer' to disk failed\n");
		return -1;
	}
