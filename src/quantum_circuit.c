...
///
int apply_quantum_matchgate_circuit(const struct matchgate gates[], const int ngates, const int wires[], const struct statevector* restrict psi, struct statevector* restrict psi_out)
{
	assert(ngates >= 1);
	...
	for (int i = 0; i < ngates; i++)
	{
		...
		apply_matchgate(&gates[i], wires[2*i], wires[2*i + 1], psi0, psi1);
	}
	...
	const struct statevector* restrict psi, struct quantum_circuit_cache* cache, struct statevector* restrict psi_out)
{
	...
	for (int i = 0; i < ngates; i++)
	{
		apply_matchgate(&gates[i], wires[2*i], wires[2*i + 1], &cache->psi_list[i],
	...
	const struct statevector* restrict dpsi_out, struct statevector* restrict dpsi, struct matchgate dgates[])
{
	...
	for (int i = ngates - 1; i >= 0; i--)
	{
		...
		apply_matchgate_backward(&gates[i], wires[2*i], wires[2*i + 1], &cache->psi_list[i], dpsi1, dpsi0, &dgates[i]);
	}
